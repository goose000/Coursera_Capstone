...
oldwd <- getwd()
setwd("C:/Users/goose/OneDrive/Documents/Data Science/Coursera/Capstone/Coursera_Capstone")

twitterFile <- file('Data/en_US/en_US.twitter.txt')
blogsFile <- file('Data/en_US/en_US.blogs.txt')
newsFile <- file('Data/en_US/en_US.news.txt', 'rb')

twitterText <- readLines(twitterFile, encoding = "UTF-8", skipNul = T)
blogsText <- readLines(blogsFile, encoding = "UTF-8", skipNul = T)
newslines<-R.utils::countLines('Data/en_US/en_US.news.txt')
newsText <- readLines(newsFile, n = newslines[1], encoding = "UTF-8", skipNul = T)
...
twitterTrainIndex <- sample(1:twitterTextLength, sampleSize*twitterTextLength)
blogsTrainIndex <- sample(1:blogsTextLength, sampleSize*blogsTextLength)
newsTrainIndex <- sample(1:newsTextLength, sampleSize*newsTextLength)

twitterTextTrain <- twitterTextTib[twitterTrainIndex,]
blogsTextTrain <- blogsTextTib[blogsTrainIndex,]
newsTextTrain <- newsTextTib[newsTrainIndex,]
...
sentencesTib <- bind_rows(twitterSentencesTib, blogsSentencesTib, newsSentencesTib)


diverseSample <- sample(1:nrow(sentencesTib), sampleSize2*nrow(sentencesTib))
...
for (n in 1:maxNGram) {
    tokensTib <- rbind(
        tokensTib, 
        tibble(n, unnest_tokens(diverseSampleTib, ngram, text, token = 'ngrams', n = n, collapse = F)))
}
...
sentencesTestTib <- bind_rows(twitterSentencesTestTib, blogsSentencesTestTib, newsSentencesTestTib)

tokensTestTib <- tibble()
for (n in 1:maxNGram) {
    tokensTestTib <- rbind(
        tokensTestTib, 
        tibble(n, unnest_tokens(sentencesTestTib, ngram, text, token = 'ngrams', n = n, collapse = F)))
}  
...
testForMatch <- function(ngramTib, ngramDictionary, nresults = 10) {
    ...
    if (ngramTib$End %in% best) which(best == ngramTib$End) else 'No Match'
}

testResults <- vector(mode = 'character', length = nrow(cleanTestTokens))
start_time <- Sys.time()
for (gram in 1:nrow(cleanTestTokens)) {
    print(gram)
    testResults[gram] <- testForMatch(cleanTestTokens[gram,], ngrams)
}
...
ngramify <- function(string, maxNGram = 5) {
    ...
    if (tokenLength<maxNGram) maxNGram <- tokenLength
    for (n in ngramSize:1){
        temp <- unnest_tokens(stringTib, text, text, token = 'ngrams', n = n) %>% 
        tail(1) %>%
        mutate(n = n + 1)
...
predictWord <- function(ngramTib, ngramDictionary = ngrams, nresults = 10) {
    best <- NULL
    maxNGram <- length(ngramDictionary)
    j <- 1
    for (i in maxNGram:1) {
        ...
        if (ngramTib$n[1]>=i){
            if(i>1){
                ...
                temp <- ngramDictionary[[i]] %>% 
                    ...
                    select(End) 
                best <- append(best,temp$End)
                best <- if(length(best)>0)best[1:min(nresults,length(best))]
                j <- j + 1
            }else{
                best <- append(best,ngramDictionary[[1]]$End)
                best <- best[1:min(nresults,length(best))]
